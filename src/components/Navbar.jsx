...
export default function Nav() {
  ...
  var logoSrc = theme === "dark" ? "/light-logo.png" : "/dark-logo.png";
    ...
    <nav className="flex items-center justify-between">
      ...
      <div className="hidden md:flex gap-2">
        {menu.map(function (item) {
          return (
            <Button key={item.name} variant="ghost" size="sm" className="text-[15px]" asChild>
      ...
      {open && (
        <div className="absolute top-13 left-0 w-full bg-white dark:bg-[#171717] shadow-md flex flex-col items-start p-5 gap-7 md:hidden">
          ...
          {menu.map(function (item) {
            return (
              <div key={item.name} onClick={() => setOpen(false)}>
                <a href={item.href}>{item.name}</a>
              </div>
            );
          })}
